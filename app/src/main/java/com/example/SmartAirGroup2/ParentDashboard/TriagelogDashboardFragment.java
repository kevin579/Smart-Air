 ...
 *
 *  - View History: Opens the historical triage log, with filtering capabilities.
 *  - Export triage : Intended to allow exporting the triage records to CSV or PDF.
 *
 * Navigation:
 ...
 *  next fragments (TriageHistoryFragment), which perform actual operations.
 *
 * Last Updated: Nov 23, 2025
 * Author: Randy Yang
 */
public class TriagelogDashboardFragment extends Fragment {

    // ───────────────────────────────
    // UI COMPONENTS
    // ───────────────────────────────
    private Toolbar toolbar;
    private CardView cardExport, cardViewHistory;
    ...
    public View onCreateView(
        ...
        View view = inflater.inflate(R.layout.fragment_child_triage_log, container, false);
        ...
        toolbar.setTitle(name + "'s Triage Log");
        ...
        cardExport.setOnClickListener(v ->{
            ExportTriagelog ExportFrag = new ExportTriagelog();
            //todo: implement export triage log when OTT is fully complete
        ...
        cardViewHistory.setOnClickListener(v -> {
            TriageHistoryFragment histFrag = new TriageHistoryFragment();
            //todo: implement view triage history when OTT is fully complete
            Bundle args = new Bundle();
            args.putString("childUname", uname);
            args.putString("childName", name);
